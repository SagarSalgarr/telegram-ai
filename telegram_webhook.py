...
async def start(update: Update, context: CustomContext) -> None:
    ...
    await send_message_to_bot(update.effective_chat.id, f" Welcome to CyberGuard Assistant", context)
...
async def bot_handler(update: Update, context: CustomContext):
    ...
    inline_keyboard_buttons = [
        ...
        [InlineKeyboardButton(button_labels["cyberguard_assistant"], callback_data='botname_cyberguard_assistant')]]
    reply_markup = InlineKeyboardMarkup(inline_keyboard_buttons)
...
def get_bot_endpoint(botName: str):
    if botName == "story":
        ...
        # return os.environ["ACTIVITY_API_BASE_URL"] + '/api/activitybot/v1/query'
...
async def get_query_response(query: str, voice_message_url: str, update: Update, context: CustomContext) -> Union[
    ...
    try:
        ...
        if selected_bot != "story":
            reqBody["input"]["context"] = selected_bot
